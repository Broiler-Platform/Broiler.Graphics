...
// partial so the core BColor definition stays compact.
public readonly partial struct BColor
{
    /// <summary>
    /// Resolves a CSS/X11 color name (case-insensitive) to an opaque color.
    /// Returns <see langword="false"/> for unknown names.
    /// </summary>
    public static bool TryGetNamedColor(string name, out BColor color)
    {
        if (!string.IsNullOrEmpty(name) && s_named.TryGetValue(name, out uint rgb))
        {
            color = new BColor((byte)((rgb >> 16) & 0xFF), (byte)((rgb >> 8) & 0xFF), (byte)(rgb & 0xFF));
            return true;
        }
        color = Empty;
        return false;
    ...
    // 0xRRGGBB (opaque). CSS Color Module Level 4 named colors, including the
    // gray/grey spelling variants and rebeccapurple.
    private static readonly Dictionary<string, uint> s_named = new(StringComparer.OrdinalIgnoreCase)
    {
